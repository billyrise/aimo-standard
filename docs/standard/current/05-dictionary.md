...
| File | Description |
| --- | --- |
| `taxonomy_dictionary_v0.1.csv` | SSOT: Full dictionary in CSV (91 codes, 21 columns) |
| `dictionary_seed.csv` | Legacy copy for API compatibility |
| `taxonomy_en.yaml` | Generated English taxonomy |
| `taxonomy_ja.yaml` | Generated Japanese taxonomy |
| `code_system.csv` | Dimension namespaces and prefixes |

**Location:** `source_pack/03_taxonomy/`

The SSOT files are the authoritative source. This documentation page is explanatory.

### Update Workflow

1. Edit `taxonomy_dictionary_v0.1.csv` (the SSOT)
2. Sync `dictionary_seed.csv` to match
3. Run `python tooling/checks/lint_taxonomy_dictionary.py` to validate
4. Run `python tooling/taxonomy/build_taxonomy_assets.py` to regenerate derived files
