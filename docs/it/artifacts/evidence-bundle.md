...
- **File obbligatori**: almeno un set di Evidence (EV) allineato con il [Template Evidence Pack (EP)](../../standard/current/06-ev-template/), un [Dizionario](../../standard/current/05-dictionary/), un breve **Riepilogo** (sintesi esecutiva del bundle) e un **Change Log** (o riferimento ad esso) per le modifiche al bundle o ai suoi contenuti.
...
| --- | --- | --- | --- | --- | --- |
| Evidence | Record EV (JSON/array) | Sì | Registro di quanto accaduto; collegamento a richiesta/revisione/eccezione/rinnovo | id, timestamp, source, summary; ref ciclo di vita opzionali | [Validator](../../validator/), aimo-ev.schema.json |
...
Per evitare doppia costruzione e ambiguità in audit, quanto segue è **normativo**: (1) I record EV (JSON) sono l'**indice/ledger** (tracciabilità verificabile da macchina). (2) I file Evidence Pack (EP-01..EP-07 e manifest) sono il **payload**. (3) I record EV DOVREBBERO riferire il payload tramite `evidence_file_ids` (es. EP-01) e/o hash; il [Validator](../../validator/) verifica l'integrità referenziale. (4) **Set minimo di sottomissione**: EV JSON + Dictionary + Summary + Change Log + Evidence Pack (zip). Vedi [Template Evidence Pack](../../standard/current/06-ev-template/) per i tipi di documento EP-01..EP-07.
...
Vedere [Requisiti Minimi di Evidence](../minimum-evidence/) per i campi MUST-level e i gruppi del ciclo di vita.
...
!!! info "Integrità e controllo degli accessi"
    ...
    Vedere [Requisiti Minimi di Evidence > Integrità & Accesso](../minimum-evidence/#6-integrity-access) per una guida dettagliata.

## Percorso di audit

Da questa pagina, il tipico percorso di audit prosegue:

1. **Prossimo**: [Requisiti Minimi di Evidence](../minimum-evidence/) — Checklist MUST-level per ciclo di vita
2. **Poi**: [Mappa di Copertura](../../coverage-map/) — Mappatura ai framework esterni
3. **Validazione**: [Validator](../../validator/) — Eseguire controlli strutturali
