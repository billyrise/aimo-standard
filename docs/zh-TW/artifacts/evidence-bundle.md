...
| --- | --- | --- | --- | --- | --- |
| 證據 | EV 記錄（JSON/陣列） | 是 | 記錄發生的事情；連結到請求/審查/例外/續期 | id、timestamp、source、summary；選用生命週期參照 | [驗證器](../validator/index.md)、aimo-ev.schema.json |
...
請參閱[最低證據要求](minimum-evidence.md)以了解必要層級欄位和生命週期群組。
...
!!! info "完整性和存取控制"
    ...
    請參閱[最低證據要求 > 完整性與存取](minimum-evidence.md#6-integrity-access)以獲取詳細指引。
