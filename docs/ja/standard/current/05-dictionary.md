...
辞書は、すべてのAIMOタクソノミーコードの**唯一の正（Single Source of Truth: SSOT）**です。言語非依存アーキテクチャを採用しています：

- **構造（canonical）**: `data/taxonomy/canonical.yaml` — コード、ステータス、ライフサイクル
- **翻訳（i18n）**: `data/taxonomy/i18n/*.yaml` — ローカライズされたラベルと定義

この分離により、新言語追加時にスキーマ変更が不要になります。
...
!!! info "正式なソース"
    **SSOT**: `data/taxonomy/canonical.yaml` + `data/taxonomy/i18n/*.yaml`  
    CSVファイルは互換性のためにSSOTから生成される**派生物**です。
...
!!! note "完全なコード一覧"
    91コードの完全なリストは`data/taxonomy/canonical.yaml`で利用可能です。ローカライズされたラベルは`data/taxonomy/i18n/{lang}.yaml`を参照してください。レガシーCSVエクスポートはリリースに含まれています。
...
後方互換性のため、EN+JA結合CSVが生成されます：

| ファイル | 説明 | ステータス |
| --- | --- | --- |
| `taxonomy_dictionary_v0.1.csv` | EN+JA列（21列） | 凍結 — 新言語追加なし |

!!! warning "新規言語列の追加なし"
    レガシーCSV形式（`*_en`, `*_ja`サフィックス）は凍結されています。追加言語（ES, DE, KO, ZH等）はこのファイルに**追加されません**。代わりに以下を使用してください：
    
    - 言語別エクスポート: `taxonomy_dictionary.{lang}.csv`（近日対応）
    - 言語パック: `data/taxonomy/i18n/{lang}.yaml`

レガシーCSVは`build_taxonomy_assets.py`により生成され、元の形式に依存するツール向けにリリースに含まれます。
