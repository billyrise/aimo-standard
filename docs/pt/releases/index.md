...
- **[Standard → Versões → Procedimento de Verificação](../standard/versions/)** — verificação passo a passo de checksums (Linux/macOS/Windows) e attestação de proveniência.
...
- **Tabela de Histórico de Versões**: [Padrão > Versões](../standard/versions/) — tabela de versões com links diretos para todos os ativos de release (PDF, ZIP, SHA256)
- **Changelog (spec)**: [Padrão > Atual > Changelog](../standard/current/08-changelog/) — histórico de mudanças normativas e não-normativas.
...
Para governança e política de versionamento, veja [Governança](../governance/).

## Preparando seu pacote de submissão

Ao preparar evidências para submissão de auditoria:

1. **Crie seu Pacote de Evidências**: Siga [Pacote de Evidências](../artifacts/evidence-bundle/) e [Requisitos Mínimos de Evidências](../artifacts/minimum-evidence/) para criar registros EV, Dicionário, Resumo e Log de Alterações.
...
Para o guia de preparação completo, veja [Trust Package](../governance/trust-package/).
...
Para o procedimento de verificação completo (checksums, attestação, passo a passo), veja **[Standard → Versões → Procedimento de Verificação](../standard/versions/)**. Veja também [Trust Package](../governance/trust-package/) para materiais prontos para auditores.
