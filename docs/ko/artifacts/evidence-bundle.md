...
| --- | --- | --- | --- | --- | --- |
| 증거 | EV 레코드 (JSON/배열) | 예 | 발생한 내용의 기록; 요청/검토/예외/갱신에 대한 링크 | id, timestamp, source, summary; 선택적 생명주기 참조 | [검증기](../validator/index.md), aimo-ev.schema.json |
...
MUST 수준 필드와 생명주기 그룹에 대해서는 [최소 증거 요구사항](minimum-evidence.md)을 참조하세요.
...
!!! info "무결성 및 접근 통제"
    ...
    자세한 지침은 [최소 증거 요구사항 > 무결성 및 접근](minimum-evidence.md#6-integrity-access)을 참조하세요.

## 감사 여정

이 페이지에서 일반적인 감사 여정은 다음과 같이 계속됩니다:

1. **다음**: [최소 증거 요구사항](minimum-evidence.md) — 생명주기별 MUST 수준 체크리스트
2. **그 다음**: [커버리지 맵](../coverage-map/index.md) — 외부 프레임워크에 대한 매핑
3. **검증**: [검증기](../validator/index.md) — 구조적 검사 실행
4. **다운로드**: [릴리스](../releases/index.md) — 릴리스 자산 획득 및 체크섬 확인
