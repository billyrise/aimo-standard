...
The dictionary is the **Single Source of Truth (SSOT)** for all AIMO taxonomy codes. It uses a language-neutral architecture:

- **Structure (canonical)**: `data/taxonomy/canonical.yaml` — codes, status, lifecycle
- **Translations (i18n)**: `data/taxonomy/i18n/*.yaml` — localized labels and definitions

This separation ensures adding new languages does not require schema changes.

The dictionary contains:

- All 91 codes across 8 dimensions
- Labels and definitions (via language packs)
...
!!! info "Source of Truth"
    **SSOT**: `data/taxonomy/canonical.yaml` + `data/taxonomy/i18n/*.yaml`  
    CSV files are **derived artifacts** generated from the SSOT for compatibility.
...
!!! note "Complete Code Listings"
    The complete list of 91 codes is available in `data/taxonomy/canonical.yaml`. For localized labels, see `data/taxonomy/i18n/{lang}.yaml`. Legacy CSV exports are available in releases.
...
For backward compatibility, a combined EN+JA CSV is generated:

| File | Description | Status |
| --- | --- | --- |
| `taxonomy_dictionary_v0.1.csv` | EN+JA columns (21 columns) | Frozen — no new languages |

!!! warning "No New Language Columns"
    The legacy CSV format (`*_en`, `*_ja` suffixes) is frozen. Additional languages (ES, DE, KO, ZH, etc.) will **not** be added to this file. Instead, use:
    
    - Per-language exports: `taxonomy_dictionary.{lang}.csv` (coming soon)
    - Language packs: `data/taxonomy/i18n/{lang}.yaml`

The legacy CSV is generated by `build_taxonomy_assets.py` and included in releases for tools that depend on the original format.
