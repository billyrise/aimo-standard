...
- **符合性**：如何声明合规以及级别的含义 — [符合性](../../conformance/)
- **覆盖映射**：与外部标准的映射 — [覆盖映射索引](../../coverage-map/)、[覆盖映射方法论](../../coverage-map/methodology/)
- **标准**：规范性要求和定义 — [标准（当前版本）](../../standard/current/)
- **分类法**：AI治理的8维分类系统 — [分类法](../../standard/current/03-taxonomy/)、[代码](../../standard/current/04-codes/)、[字典](../../standard/current/05-dictionary/)
- **证据包**：结构、目录、可追溯性 — [证据包](../../artifacts/evidence-bundle/)
- **最低证据要求**：按生命周期的 MUST 级别清单 — [最低证据要求](../../artifacts/minimum-evidence/)
- **验证器**：规则和参考检查 — [验证器](../../validator/)
- **示例**：审计就绪的示例包 — [示例](../../examples/)
- **发布**：变更历史和分发 — [发布](../../releases/)
- **治理**：政策、安全、许可 — [治理](../../governance/)

## 审计就绪的最小集

| 项目 | 在哪里找到 | 结果/证明什么 |
| --- | --- | --- |
| 符合性级别 | [符合性](../../conformance/) | 如何声明合规以及所需证据的范围 |
| 覆盖映射 | [覆盖映射索引](../../coverage-map/)、[覆盖映射方法论](../../coverage-map/methodology/) | 针对外部法规和标准的可解释性 |
| 分类法与字典 | [分类法](../../standard/current/03-taxonomy/)、[代码](../../standard/current/04-codes/)、[字典](../../standard/current/05-dictionary/) | AI系统的分类系统（8个维度、91个代码） |
| 证据工件 | [证据包](../../artifacts/evidence-bundle/)、[最低证据](../../artifacts/minimum-evidence/)、[EV 模板](../../standard/current/06-ev-template/) | 支持可追溯性必须存在的数据 |
| 验证器检查 | [验证器](../../validator/) | 如何验证内部一致性和完整性 |
| 示例包 | [示例](../../examples/) | 实践中审计就绪包的样子 |
| 变更控制 | [发布](../../releases/)、[治理](../../governance/) | 如何管理和传达更新 |
| 安全/许可证/商标 | [治理](../../governance/) | 采用决策的法律和安全态势 |

## 如何引用

使用仓库 README 获取引用指南和背景；治理链接到权威政策。
请参阅 [README.md](https://github.com/billyrise/aimo-standard/blob/main/README.md) 和 [治理](../../governance/)。
...
有关完整的范围定义、假设和采用者责任，请参阅 [责任边界](../responsibility-boundary/)。

## 如何准备提交包

按照以下步骤准备审计就绪的提交：

1. **生成证据包**：按照 [证据包](../../artifacts/evidence-bundle/) 和 [最低证据要求](../../artifacts/minimum-evidence/) 创建 EV 记录、字典、摘要和变更日志。
...
有关范围、假设和采用者责任的详情，请参阅 [责任边界](../responsibility-boundary/)。
...
!!! success "构建来源可用"
    所有发布资产包括加密签名的构建证明。请参阅 [验证程序](../../standard/versions/#4-verify-build-provenance-attestation) 了解证明验证步骤。
...
1. **分类系统**：[分类法](../../standard/current/03-taxonomy/) + [字典](../../standard/current/05-dictionary/) — 理解8维代码系统
2. **证据结构**：[证据包](../../artifacts/evidence-bundle/) — 理解包目录和可追溯性
3. **必需证据**：[最低证据要求](../../artifacts/minimum-evidence/) — 按生命周期的 MUST 级别清单
4. **框架对齐**：[覆盖映射](../../coverage-map/) + [方法论](../../coverage-map/methodology/) — 了解 AIMO 如何映射到外部框架
5. **验证**：[验证器](../../validator/) — 运行结构一致性检查
