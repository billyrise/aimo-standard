...
- **必需文件**：至少一个与 [Evidence Pack 模板（EP）](../../standard/current/06-ev-template/) 对齐的证据（EV）集、一个 [字典](../../standard/current/05-dictionary/)、一个简短的 **摘要**（包的执行摘要）以及一个 **变更日志**（或其引用），用于记录包或其内容的变更。
...
| --- | --- | --- | --- | --- | --- |
| 证据 | EV 记录（JSON/数组） | 是 | 记录发生的事情；链接到请求/审查/例外/续期 | id, timestamp, source, summary; 可选生命周期引用 | [验证器](../../validator/), aimo-ev.schema.json |
...
请参阅 [最低证据要求](../minimum-evidence/) 了解 MUST 级别字段和生命周期组。
...
!!! info "完整性和访问控制"
    ...
    请参阅 [最低证据要求 > 完整性与访问](../minimum-evidence/#6-integrity-access) 获取详细指南。

## 审计旅程

从此页面开始，典型的审计旅程继续：

1. **下一步**：[最低证据要求](../minimum-evidence/) — 按生命周期的 MUST 级别清单
2. **然后**：[覆盖映射](../../coverage-map/) — 与外部框架的映射
3. **验证**：[验证器](../../validator/) — 运行结构检查
